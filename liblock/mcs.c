...
struct liblock_impl {
    ...
    char __pad[pad_to_cache_line(sizeof(pthread_mutex_t) + sizeof(void*))];
};
...
__thread struct mcs_node* my_node = NULL;

static void lock_mcs(struct liblock_impl *impl)
{
    volatile struct mcs_node *tail, *me = my_node;
...
{
    volatile struct mcs_node *me = my_node;
...
{
    munmap((void *)my_node, r_align(sizeof(struct mcs_node), PAGE_SIZE));
}
