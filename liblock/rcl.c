...
/* No local CAS? */
int liblock_rcl_no_local_cas = 0;
...
/*
    rclprintf(server, "CREATE context %p with stack at %p and size %d",
...
static void insert_in_ready_and_remove_from_timed(struct fqueue *node)
{
    ...
    /*
        rclprintf(server, "++++      reinjecting mini thread: %p",
                  mini_thread);
    ...
    */
    ...
    /*
        rclprintf(server, "++++      reinjecting mini thread: %p done",
                  mini_thread);
        ll_print("ready", &server->mini_thread_ready);
    */
}
...
static void *do_liblock_execute_operation(rcl)(liblock_lock_t *lock,
                                               void *(*pending)(void *),
                                               void *val)
{
    ...
    /*
        rclprintf(server, "*** sending operation %p::%p for client %d - %p",
...
static int servicing_loop_slow_path(struct server *server, int time)
{
    ...
    if(next)
    {
        ...
        /*
        rclprintf(server, "servicing-loop::elect mini-thread: %p "
        ...
        */
        ...
        /*
        rclprintf(server, "servicing-loop::mini-thread: %p is up",
                  me->mini_thread);
        */
    ...
    else if(server->nb_free_threads > 0)
    {
        ...
        {
            /*
            rclprintf(server, "servicing::gona sleep %p %p %p", me, &me->ll,
                      me->ll.content);
            */
            __sync_fetch_and_sub(&server->nb_ready_and_servicing, 1);
        ...
        static int z = 0;
        if(!(++z % 200000))
            rclprintf(server, "servicing-loop::yield processor");
        */
...
static void servicing_loop()
{
    ...
    // rclprintf(server, "::: start servicing loop %p %x", me->mini_thread,
    //           pthread_self());

    // schedctl_start(schedctl_init());

    if (!liblock_rcl_no_local_cas)
    {
        do {
            me->timestamp = server->timestamp;
            server->alive = 1;
    ...
    // rclprintf(server, "::: end of servicing loop %p", me->mini_thread);
...
static void ensure_at_least_one_free_thread(struct server *server)
{
    ...
    if(server->nb_free_threads < 1)
    {
        ...
        {
            /*
            rclprintf(server, "no more free threads %d",
            ...
            */
            ...
            else
            {
                ...
                /*
                rclprintf(server, "CREATE a new servicing thread %p with stack"
            ...
            else
            {
               ...
               /*
                rclprintf(server, "launching the new servicing thread %p",
                          elected);
                */

                liblock_thread_create_and_bind(server->hw_thread, "rcl",
                                               &elected->tid, &attr,
                                               servicing_thread, elected);

                /*
                rclprintf(server, "launching of the servicing thread %p done",
                          elected);
                */
            }
        }
...
static void *manager_thread(void *arg)
{
    ...
    while(server->state == SERVER_UP)
    {
        ...
        if(!server->alive)
        {
            // rclprintf(server, "no more live servicing threads");
...
endloop:
    ...
    // rclprintf(server, "manager thread over");
...
static void launch_server(struct server *server, void (*callback)(void))
{
    ...
    liblock_thread_create_and_bind(server->hw_thread, "rcl", &tid, 0,
...
static void destroy_server(struct server *server)
{
    ...
    if(server->state == SERVER_UP)
    {
        ...
        {
            /*
            rclprintf(lock->server,
                      "launch_server::waiting the end of the server");
            */

            pthread_cond_wait(&server->cond_state, &server->lock_state);

            /*
            rclprintf(impl->server,
                      "launch_server::waiting the end of server done");
            */
        }
    }
...
static int cond_signal(liblock_cond_t *cond)
{
    ...
    if(node)
    {
        ...
        if(mini_thread->is_timed)
            fqueue_remove((struct fqueue**)&mini_thread->wait_on->impl.data,
                          &mini_thread->ll_timed, 0);
...
static int do_liblock_cond_timedwait(rcl)(liblock_cond_t *cond,
                                          liblock_lock_t *lock,
...
static struct liblock_impl *do_liblock_init_lock(rcl)
                                                (liblock_lock_t *lock,
                                                 struct hw_thread *hw_thread,
                                                 pthread_mutexattr_t *attr)
{
    ...
    liblock_reserve_hw_thread_for(hw_thread, "rcl");
...
static void do_liblock_run(rcl)(void (*callback)())
{
    ...
    for(i = 0; i < topology->nb_hw_threads; i++)
    {
        ...
        {
            /*
            rclprintf(servers[i], "++++ launching: %d from %d", i,
                      sched_getcpu());
            */
            ...
            /*
            rclprintf(servers[i], "++++ launched: %d from %d", i,
                      sched_getcpu());
            */
        }
...
static void do_liblock_declare_server(rcl)(struct hw_thread *hw_thread)
{
...
static void do_liblock_cleanup(rcl)()
{}
