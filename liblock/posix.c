...
static struct liblock_impl* do_liblock_init_lock(posix)(liblock_lock_t* lock,
                                                        struct hw_thread* server,
...
static void* do_liblock_execute_operation(posix)(liblock_lock_t* lock,
                                                 void* (*pending)(void*),
                                                 void* val)
{
...
static int do_liblock_cond_wait(posix)(liblock_cond_t* cond,
                                       liblock_lock_t* lock)
{
    return pthread_cond_wait(&cond->impl.posix_cond, &lock->impl->posix_lock);
}

static int do_liblock_cond_timedwait(posix)(liblock_cond_t* cond,
                                            liblock_lock_t* lock,
                                            const struct timespec* ts)
{
    return pthread_cond_timedwait(&cond->impl.posix_cond,
                                  &lock->impl->posix_lock,
                                  ts);
}
...
static void do_liblock_declare_server(posix)(struct hw_thread* core)
{}

static void do_liblock_cleanup(posix)(void)
{
